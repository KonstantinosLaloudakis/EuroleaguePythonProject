...
class ClutchAnalysis:
    ...
    def fetch_clutch_shots(self):
        ...
        full_df['MINUTE'] = pd.to_numeric(full_df['MINUTE'], errors='coerce').fillna(0)
        full_df['POINTS_A'] = pd.to_numeric(full_df['POINTS_A'], errors='coerce').fillna(0)
        full_df['POINTS_B'] = pd.to_numeric(full_df['POINTS_B'], errors='coerce').fillna(0)
        
        full_df['ScoreMargin'] = abs(full_df['POINTS_A'] - full_df['POINTS_B'])
        
        # Clutch Filter: Minute > 35 (last 5 mins of 40) + OT, Score <= 5
        clutch_shots = full_df[
            (full_df['MINUTE'] > 35) & 
            (full_df['ScoreMargin'] <= 5)
        ].copy()
        
        print(f"Found {len(clutch_shots)} clutch shots.")
...
if __name__ == "__main__":
    # Example usage: Analyze 2024
    analysis = ClutchAnalysis(2007, 2024)
    analysis.run()
